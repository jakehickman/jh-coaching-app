    ...
    <div className="space-y-4">
      ...
      {days.map((day: any, i: number) => (
        <Card key={i} className="overflow-hidden">
          <button
            onClick={() => setExpandedDays(prev => { const next = new Set(prev); if (next.has(i)) next.delete(i); else next.add(i); return next; })}
...
function WorkoutLogTab() {
  ...
  const saveMutation = trpc.workoutSessions.save.useMutation({
    onSuccess: () => {
      ...
      void utils.workoutSessions.list.invalidate();
      void utils.dailyLog.list.invalidate();
      setSaving(false);
  ...
  const deleteMutation = trpc.workoutSessions.delete.useMutation({
    onSuccess: () => { void utils.workoutSessions.list.invalidate(); setDeleting(null); toast.success("Session deleted."); },
  ...
  const updateDateMutation = trpc.workoutSessions.updateDate.useMutation({
    onSuccess: () => { void utils.workoutSessions.list.invalidate(); setChangingDateId(null); toast.success("Session date updated."); },
