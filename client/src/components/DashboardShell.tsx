...
export default function DashboardShell({ children, mode }: DashboardShellProps) {
  ...
  // ─── Client layout: mobile-first, max-width capped, bottom-friendly ─────────
  return (
    <div className="min-h-screen bg-background flex">
      {/* Mobile overlay */}
      {mobileOpen && (
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-6 border-b border-border">
          <div>
            ...
            <p className="text-base font-bold text-foreground mt-0.5">Client Dashboard</p>
          </div>
          <button
            className="lg:hidden text-muted-foreground hover:text-foreground"
            onClick={() => setMobileOpen(false)}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {nav.map((item) => {
            const isActive =
              location === item.href ||
              (item.href !== "/dashboard/overview" && location.startsWith(item.href));
            return (
              <Link
                key={item.href}
                href={item.href}
