 ...
 * where exercises have weightUnit='kg' (incorrectly saved due to wrong default).
 *
 * Run: node scripts/backfill-jake-weight-unit-v2.mjs
 */
...
console.log('Users found:', users.map(u => `${u.id}: ${u.name} (${u.email})`));
...
// 3. For each session, update exercises that have weightUnit='kg' to 'lbs'
